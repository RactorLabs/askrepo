...
impl RactorClient {
    pub fn new(config: &Config) -> Result<Self> {
        ...
        let mut base_url = Url::parse(&config.ractor_host_url).or_else(|_| {
            Url::parse(&format!("http://{}", config.ractor_host_url))
        })
        .context("RACTOR_HOST_URL is not a valid URL (expected absolute URL, e.g., http://localhost:9000)")?;
    ...
    fn default_headers(token: &str) -> Result<header::HeaderMap> {
        ...
        headers.insert(
            ...
            header::HeaderValue::from_str(&format!("Bearer {}", token))
                .context("invalid session admin token for header")?,
        );
        Ok(headers)
    }
    ...
    pub async fn session_exists(&self, name: &str) -> Result<bool> {
        ...
        let res = self
            ...
            .send()
            .await
            .with_context(|| format!("failed to query session API for session '{}'", name))?;
        match res.status() {
            ...
            status => {
                ...
                Err(anyhow!(
                    "session API returned {} while checking session existence (body: {})",
                    status,
                    body
                ))
            }
    ...
    pub async fn create_session(&self, payload: &NewSessionPayload) -> Result<()> {
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow!(
                "session API returned {} when creating session (body: {})",
                status,
                body
            ));
        }

        debug!(session = %payload.name, "created session via session API");
        Ok(())
    }
}
